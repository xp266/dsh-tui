...
assert.ok(compareVersions(floor, parseUpstreamVersion('0.1.5-rc.2')!) < 0)
...
assert.ok(isCompatibleUpstreamVersion('0.1.5-rc.1'))
assert.ok(isCompatibleUpstreamVersion('0.1.5-rc.2'))
assert.ok(!isCompatibleUpstreamVersion('0.1.5-alpha.2'))
...
assert.deepEqual(installedUpstreamLines(compatibleTree), ['0.1.5-rc.2'])
...
assert.deepEqual(upstreamDriftSummary({ '@deepseek-ai/dsh-llm': '0.1.2-rc.2' }), { kind: 'older', versions: ['0.1.2-rc.2'] })
assert.deepEqual(upstreamDriftSummary({ '@deepseek-ai/dsh-llm': '0.1.2-rc.2', '@deepseek-ai/dsh-session': '0.2.0-rc.1' }), {
  kind: 'mixed',
  versions: ['0.1.2-rc.2', '0.2.0-rc.1'],
})
