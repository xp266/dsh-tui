  ...
  async function listSessions(): Promise<SessionSummary[]> {
    ...
    if (query !== undefined && typeof query.listSessions === 'function') {
      ...
      for (const record of allRecords) {
        ...
        if (live !== undefined) {
          ...
          if (blank && record.header.id !== String(activeAgent.id)) continue
        ...
        if (cached !== undefined) {
          if (cached.blank && record.header.id !== String(activeAgent.id)) continue
      ...
      if (query.readTitleSnapshots !== undefined && largeRecords.length > 0) {
        ...
        for (let i = 0; i < largeRecords.length; i++) {
          ...
          const title = snapshot !== undefined && snapshot.title !== undefined && snapshot.title !== ''
            ? snapshot.title
            : undefined
          sessionMetaCache.set(record.header.id, { title, blank: false })
          if (title !== undefined) titleBySession.set(record.header.id, title)
        }
      }
      const records = [...smallRecords, ...largeRecords]
      return records
        .map(record => ({
          id: record.header.id,
    ...
    return ctx.sessions
      ...
      .filter(session => {
        ...
        return !blank || session.id === activeAgent.id
      })
      .map(session => ({
